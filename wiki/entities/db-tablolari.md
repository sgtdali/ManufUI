...
## manuf_press_mold_changes

Pres hücresi kalıp değişim geçmişi ve parça adetlerini takip etmek için kullanılan manuel kalıp değişim kayıtları.

| Kolon | Tip | Açıklama |
|-------|-----|----------|
| id | uuid | PK |
| tarih | date | Kalıp değişim tarihi |
| zaman_dilimi | text | Kalıp değişim saati dilimi |
| sira_no | integer | Zaman dilimi sırası (1–9 veya 1–8) |
| sokulen_kalip | text | Sökülen kalıp adı (Manuel kayıtlarda null) |
| takilan_kalip | text | Değiştirilen kalıp adı (Seçilen kalıp buraya yazılır) |
| description | text | Açıklama / Not |
| created_at | timestamptz | Kayıt oluşturulma zamanı |
| updated_at | timestamptz | Son güncelleme zamanı |

**Unique constraint**: `(tarih, sira_no)` — aynı saat diliminde tek bir kalıp değişimi girilebilir.
