...
import { Plus, Trash2, ChevronDown, ChevronRight, CloudOff, RefreshCw } from "lucide-react";
...
export function ActionRow({
  ...
  onDueDateChange, onDelete, onAddSub, onCreateSub, onCloseSub,
      ...
      <tr className={`hover:bg-zinc-50 ${depth > 0 ? "bg-zinc-25" : ""}`}>
        ...
        <td className="px-3 py-3">
          <div className="flex items-center gap-1.5" style={{ paddingLeft: depth > 0 ? depth * 12 : 0 }}>
            <span className={`font-medium ${item.status === "Tamamlandı" ? "text-zinc-400 line-through" : "text-zinc-900"}`}>
              {item.title}
            </span>
