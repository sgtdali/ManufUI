...
import { shareReport } from "./_components/shareReport";
...
export default function DashboardyPage() {
  ...
  const handleShare = () => shareReport(startDate, endDate, setSharing);
      ...
      <header className="bg-white border-b border-zinc-200/80 sticky top-0 z-40 backdrop-blur-md bg-white/95">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {!isReadOnly && (
              <>
                <Link href="/" className="p-2 text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100 rounded-lg transition-all" title="Form Sayfasına Dön">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-bold tracking-tight text-zinc-900">Detaylı Performans Paneli</h1>
            </div>
          </div>
          {!isReadOnly && (
            <div className="flex items-center gap-2">
              <button onClick={handleShare} disabled={sharing || loading} className="text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 px-3.5 py-2 rounded-lg transition-all border border-indigo-700 disabled:border-indigo-500 flex items-center gap-1.5 shadow-xs cursor-pointer disabled:cursor-not-allowed active:scale-95 disabled:scale-100">
                {sharing ? (<><RotateCw className="h-3.5 w-3.5 animate-spin" />Hazırlanıyor...</>) : (<><Share2 className="h-3.5 w-3.5" />Raporu Paylaş</>)}
              </button>
              <Link href="/dashboard" className="text-xs font-semibold text-zinc-600 hover:text-zinc-900 px-3 py-2 rounded-lg hover:bg-zinc-100 transition-all border border-zinc-200 bg-zinc-50">Standart OEE Dashboard</Link>
            </div>
          )}
        </div>
      </header>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              <h2 className="text-base font-bold text-zinc-800 flex items-center gap-2">
                <Calendar className="h-4 w-4 text-zinc-500" />Tarih Aralığı Seçimi
              </h2>
              <p className="text-xs text-zinc-500 mt-1">Seçilen tarih aralığındaki gerçekleşen toplam üretim adetlerini görüntülersiniz.</p>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <button onClick={() => setPreset(7)} className="text-xs px-3 py-1.5 font-medium rounded-lg border border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900 active:scale-95 transition-all">Son 7 Gün</button>
              <button onClick={() => setPreset(30)} className="text-xs px-3 py-1.5 font-medium rounded-lg border border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900 active:scale-95 transition-all">Son 30 Gün</button>
              <button onClick={() => setPreset(0)} className="text-xs px-3 py-1.5 font-medium rounded-lg border border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900 active:scale-95 transition-all">Bu Ay</button>
            </div>
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2">
                <label className="text-xs font-bold text-zinc-600">Başlangıç:</label>
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="px-3 py-1.5 border border-zinc-200 rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
              </div>
              <div className="flex items-center gap-2">
                <label className="text-xs font-bold text-zinc-600">Bitiş:</label>
                <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="px-3 py-1.5 border border-zinc-200 rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
              </div>
              <button onClick={() => fetchData(startDate, endDate)} className="p-2 border border-zinc-200 rounded-lg hover:bg-zinc-50 active:scale-95 transition-all" title="Yenile">
        ...
        <div id="performance-table-container" className="space-y-8 p-4 -m-4 rounded-3xl bg-[#fafafa]">
          <section className="bg-[#18174f] text-white rounded-[24px] border border-[#232168] p-7 shadow-lg relative overflow-hidden">
            <div className="absolute right-0 bottom-0 w-[55%] h-[85%] pointer-events-none select-none">
              <svg className="w-full h-full opacity-60" viewBox="0 0 300 150" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
                <path d="M 20 120 L 120 50 L 195 105 L 290 15" stroke="#2d2b7c" strokeWidth="16" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M 20 120 L 120 50 L 195 105 L 290 15" stroke="#3d3ab2" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" className="opacity-70" />
              </svg>
            </div>
            <div className="relative z-10 flex flex-col gap-4">
              <div>
                <span className="text-[10px] uppercase font-bold tracking-widest text-[#9caaf5] bg-[#201d6d]/60 px-3.5 py-1.5 rounded-full border border-[#3e3ab4]/70">Genel İlerleme Durumu</span>
                <h2 className="text-4xl font-semibold mt-4 flex items-baseline">
                  <span className="font-serif text-white">% {overallPercentage}</span>
                  <span className="text-lg font-normal text-[#8d8ab7] ml-2 font-sans">(%{targetPercentage})</span>
                </h2>
              </div>
              <div className="w-full mt-2">
                <div className="h-5 w-full bg-[#11103c]/90 rounded-full overflow-hidden border border-[#2d2b70] p-[3px]">
                  <div className="h-full bg-[#1ada93] rounded-full transition-all duration-500 ease-out" style={{ width: `${overallPercentage}%` }} />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="relative bg-white rounded-2xl border border-zinc-200/80 overflow-hidden shadow-sm">
            {loading && (<div className="absolute inset-0 bg-white/40 backdrop-blur-xs flex items-center justify-center z-10" />)}
                    ...
                    return (
                      <tr key={cell} className="hover:bg-zinc-50/50 transition-colors group">
                        <td className="py-4 px-6">
                          {isReadOnly ? (
                            <span className="font-bold text-sm text-zinc-800 block">{cell}</span>
                          ) : (
                            <Link href={linkUrl} className="font-bold text-sm text-zinc-800 hover:text-indigo-600 hover:underline transition-colors block">{cell}</Link>
                          )}
                        </td>
                        ...
                        <td className="py-4 px-6 text-center font-bold text-sm text-[#0c0a09] bg-zinc-50/30">{targetedQty.toLocaleString("tr-TR")}</td>
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-4 max-w-md">
                            ...
                            <div className="h-2 flex-1 bg-zinc-100 rounded-full overflow-hidden border border-zinc-200/30 p-[1px]">
                              <div className={`h-full bg-gradient-to-r ${progressColorClass} rounded-full transition-all duration-500 ease-out`} style={{ width: `${Math.min(percentage, 100)}%` }} />
                            </div>
                          </div>
                        </td>
                      </tr>
