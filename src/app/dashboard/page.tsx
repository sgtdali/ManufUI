...
import { DURUS_KOLONLARI } from "@/lib/types";
import {
...
import { MetricCard, SmallStat } from "./_components/MetricCard";
...
export default async function DashboardPage({
  ...
  const totalProduction = records.reduce(
    (total, record) => total + getRecordProduction(record), 0
  );
  const totalDowntime = records.reduce(
    (total, record) => total + getRecordDowntime(record), 0
  );
  ...
  const maxDepartmentProduction = Math.max(
    ...departmentSummaries.map((summary) => summary.uretim), 1
  );
  const downtimeBreakdown = DOWNTIME_KEYS.map((key) => {
    ...
    return {
      ...
      value: records.reduce(
        (total, record) => {
          return total + (record.manuf_production_rows ?? []).reduce(
            (s, row) => s + (typeof row[key] === "number" && Number.isFinite(row[key]) ? (row[key] as number) : 0), 0
          );
        }, 0
      ),
    };
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <header className="flex flex-col gap-4 border-b border-zinc-200 pb-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-normal text-zinc-950">Üretim dashboard</h1>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link className="rounded-md border border-emerald-700 bg-emerald-700 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-emerald-800" href="/api/export">Excel indir</Link>
          </div>
        </header>

        <section className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
          <form action="/dashboard" className="grid gap-3 md:grid-cols-[1fr_1fr_auto_auto]">
            <div className="space-y-1">
              <label className="text-sm font-medium text-zinc-700" htmlFor="baslangic">Başlangıç tarihi</label>
              <input className="h-9 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm outline-none focus:border-emerald-600 focus:ring-3 focus:ring-emerald-600/20" defaultValue={startDate} id="baslangic" name="baslangic" type="date" />
            </div>
            <div className="space-y-1">
              <label className="text-sm font-medium text-zinc-700" htmlFor="bitis">Bitiş tarihi</label>
              <input className="h-9 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm outline-none focus:border-emerald-600 focus:ring-3 focus:ring-emerald-600/20" defaultValue={endDate} id="bitis" name="bitis" type="date" />
            </div>
            <button className="self-end rounded-md border border-emerald-700 bg-emerald-700 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-emerald-800" type="submit">Uygula</button>
            <Link className="self-end rounded-md border border-zinc-300 bg-white px-4 py-2 text-center text-sm font-medium text-zinc-800 shadow-sm hover:bg-zinc-100" href="/dashboard">Tüm veriler</Link>
          </form>
        </section>

        {error ? (
          <section className="rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800">Veri okunamadı: {error.message}</section>
        ) : null}
        ...
        <section className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
          <div className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <span className="rounded-md bg-amber-100 px-3 py-1 text-xs font-medium text-amber-900">{departmentSummaries.length} bölüm</span>
            </div>
                      ...
                      <td className="py-3 pl-3">
                        <div className="flex items-center gap-3">
                          <div className="h-2 w-full min-w-32 rounded-md bg-zinc-100">
                            <div className="h-2 rounded-md bg-emerald-600" style={{ width: `${Math.max(percent(summary.uretim, maxDepartmentProduction), 4)}%` }} />
                          </div>
                          <span className="w-10 text-right text-xs text-zinc-500">{percent(summary.netSure, summary.kayit * DAILY_PLANNED_MINUTES)}%</span>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              {downtimeBreakdown.map((item) => (
                <div key={String(item.key)}>
                  ...
                  <div className="h-2 rounded-md bg-zinc-100">
                    <div className="h-2 rounded-md bg-rose-600" style={{ width: `${Math.max(percent(item.value, maxDowntime), 4)}%` }} />
                  </div>
                </div>
              ))}
              {downtimeBreakdown.length === 0 ? (
                <p className="rounded-lg bg-zinc-50 p-4 text-sm text-zinc-500">Duruş kaydı bulunmuyor.</p>
              ) : null}
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
          <div className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
            ...
            <p className="mb-4 text-sm text-zinc-500">{latestDate ? formatDate(latestDate) : "Kayıt yok"}</p>
            <div className="grid grid-cols-2 gap-3">
              ...
              <SmallStat label="Kullanım" value={`${percent(latestRecords.length * DAILY_PLANNED_MINUTES - latestDowntime, latestRecords.length * DAILY_PLANNED_MINUTES)}%`} />
            </div>
          </div>
